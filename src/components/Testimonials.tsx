...
const Testimonials = () => {
  ...
  useEffect(() => {
    const calculateMaxHeight = () => {
      ...
      const heights = cardsRef.current.filter(Boolean).map(card => card?.offsetHeight || 0);
  ...
  return <div className="py-16 relative">
      <div className="max-w-5xl mx-auto relative">
        ...
        carouselApi.current = api;
          ...
          <CarouselContent>
            {testimonials.map((testimonial, index) => <CarouselItem key={testimonial.id} className="sm:basis-11/12 md:basis-4/5 pl-4">
                <motion.div initial={{
              opacity: 0
            }} whileInView={{
              opacity: 1
              ...
              height: !isMobile && maxHeight > 0 ? `${maxHeight}px` : 'auto'
                  ...
                  <Card className="bg-[#1B1B1B] overflow-hidden shadow-[0_0_25px_rgba(255,255,255,0.05)] h-full flex flex-col border-0" ref={el => cardsRef.current[index] = el}>
                    <div className="p-4 md:p-8 flex flex-col h-full px-[55px]">
                      {/* User information */}
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
                        <div className="relative mb-3 sm:mb-0">
                          <Avatar className="h-14 w-14 rounded-none">
                            <AvatarImage src={testimonial.image} alt={testimonial.name} className="rounded-none" />
          ...
          <div className="flex justify-center gap-2 mt-8">
            <CarouselPrevious className="relative inset-0 translate-y-0 bg-zinc-900 border-none hover:bg-zinc-800 text-white shadow-[0_0_15px_rgba(255,255,255,0.07)]" onClick={() => handleManualNavigation('prev')} />
            <CarouselNext className="relative inset-0 translate-y-0 bg-zinc-900 border-none hover:bg-zinc-800 text-white shadow-[0_0_15px_rgba(255,255,255,0.07)]" onClick={() => handleManualNavigation('next')} />
          </div>
        </Carousel>
      </div>
    </div>;
