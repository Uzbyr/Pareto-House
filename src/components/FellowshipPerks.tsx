import { UserRoundCog, Users, PiggyBank, Cloud, TrendingUp, ArrowRight, School } from "lucide-react";
import { Link } from "react-router-dom";
import { motion } from "framer-motion";
import { useState } from "react";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from "./ui/dialog";
const targetUniversities = {
  "United States": ["Harvard University", "Stanford University", "MIT", "Princeton University", "Yale University", "Columbia University", "UC Berkeley", "University of Chicago", "University of Pennsylvania", "Cornell University"],
  "United Kingdom": ["University of Oxford", "University of Cambridge", "Imperial College London", "University College London", "London School of Economics"],
  "Canada": ["University of Toronto", "McGill University", "University of British Columbia", "University of Waterloo"],
  "France": ["École Polytechnique", "HEC Paris", "Sciences Po", "École Normale Supérieure"],
  ...
  "India": ["Indian Institute of Technology (IITs)", "Indian Institute of Management (IIMs)"],
  "Israel": ["Technion – Israel Institute of Technology", "Hebrew University of Jerusalem"]
};
const perks = [{
...
const FellowshipPerks = () => {
  ...
  return <div className="bg-black/5 dark:bg-white/5 py-32">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {perks.map((perk, index) => <motion.div key={index} initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
          ...
          delay: index * 0.1
              ...
              {index === 1 ? <div>
                  <p className="text-black/70 dark:text-white/70">
                    {perk.description}{" "}
                    <button onClick={() => setShowUniversities(true)} className="text-pareto-pink font-medium hover:underline">university ecosystem</button>
                  </p>
                </div> : <p className="text-black/70 dark:text-white/70">{perk.description}</p>}
            </motion.div>)}
        </div>
      ...
      <Dialog open={showUniversities} onOpenChange={setShowUniversities}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
            {Object.entries(targetUniversities).map(([country, universities]) => <div key={country} className="bg-black/5 dark:bg-white/5 p-4 rounded-lg">
                <h3 className="font-semibold text-lg mb-2">{country}</h3>
                <ul className="space-y-1">
                  {universities.map((university, index) => <li key={index} className="text-sm text-black/70 dark:text-white/70">
                      {university}
                    </li>)}
                </ul>
              </div>)}
