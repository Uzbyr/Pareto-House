...
export const collectRealMetrics = (): SiteMetrics => {
  ...
  const visitorsTotal =
    visitorData.total ||
    visitorsData.reduce((sum, day) => sum + day.visitors, 0);
  ...
  const funnelStages = [
    { name: "Landing Page View", value: visitorsTotal },
    {
      name: "Form Started",
      value: Math.max(
        Math.floor(visitorsTotal * formStartRate),
        totalApplications + 5,
      ),
    },
    {
      name: "Personal Info Completed",
      value: Math.max(
        Math.floor(visitorsTotal * formStartRate * 0.75),
        totalApplications + 3,
      ),
    },
    {
      name: "Education Info Completed",
      value: Math.max(
        Math.floor(visitorsTotal * formStartRate * 0.56),
        totalApplications + 2,
      ),
    },
    {
      ...
      value: Math.max(
        Math.floor(visitorsTotal * formStartRate * 0.4),
        totalApplications + 1,
      ),
    },
    {
      name: "Video Uploaded",
      value: Math.max(
        Math.floor(visitorsTotal * formStartRate * 0.3),
        totalApplications,
      ),
    },
  ...
  return {
    ...
    visitorsByDate: visitorsData.map(({ date, visitors }) => [date, visitors]),
